...
export function validateRadar(obj: unknown): ValidationResult {
  ...
  if (!isStr(radar.marketDate) || !/^\d{4}-\d{2}-\d{2}$/.test(radar.marketDate as string)) {
  ...
  ;(radar.items as unknown[]).forEach((it, i) => {
    const tag = `items[${i}]`
    if (typeof it !== 'object' || it === null) { errors.push(`${tag} inválido`); return }
    ...
    if (typeof item.metrics !== 'object' || item.metrics === null) errors.push(`${tag}.metrics ausente`)
    if (!(item.score === null || typeof item.score === 'number')) errors.push(`${tag}.score inválido`)
    ...
    if (typeof item.quality !== 'object' || item.quality === null) errors.push(`${tag}.quality ausente`)
