...
import type { Quality, Metrics } from './types'
...
export function computeMetrics(bars: Bar[]): ComputedMetrics {
  if (!Array.isArray(bars) || bars.length === 0) {
    return {
      ...
      ret_60d: null, ret_120d: null, rangePos_60d: null, upDays_5: null
    }
  }
...
export function qualityFlags(bars: Bar[], ctx: BuildItemCtx = {}): Quality {
  ...
  if (!Array.isArray(bars) || bars.length === 0) {
    return {
      partialSession: false, missingBars: true,
      staleLastBar: false, flatRange: false, symbolError: true
    }
  }
...
export function computeScore(metrics: ComputedMetrics | null): number | null {
  ...
  const posComponent =
    rangePos_60d === null ? null : clamp((rangePos_60d - 50) * 2, -100, 100)

  // componente 3: persistência curta
  const persComponent =
    upDays_5 === null ? null : clamp(((upDays_5 - 2.5) / 2.5) * 100, -100, 100)
...
export function classifyRegime(item: ClassifyInput, threshold: number | null = null): RegimeResult {
  ...
  if (ret_20d === null || ret_20d === undefined || rangePos_60d === null || rangePos_60d === undefined) {
    const alert =
      threshold !== null && typeof ret_1d === 'number'
        ? Math.abs(ret_1d) > threshold
        : false
    return {
      regime: (ret_20d === null || ret_20d === undefined) && (rangePos_60d === null || rangePos_60d === undefined)
        ? 'SEM_DADO'
        : 'NEUTRO',
      alert
    }
...
export function buildItem(symbolMeta: SymbolMeta, bars: Bar[], ctx: BuildItemCtx = {}): AnalyticsItem {
