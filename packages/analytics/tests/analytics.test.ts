...
describe("classifyRegime", () => {
  ...
  it("VIX invertido: três faixas", () => {
    ...
    expect(classifyRegime({ kind: "vix", last: 12 })).toEqual({
      regime: "TRANQUILO",
      alert: false,
    });
    expect(classifyRegime({ kind: "vix", last: null })).toEqual({
      regime: "SEM_DADO",
      alert: false,
    });
  });
...
describe("qualityFlags", () => {
  ...
  it("partialSession quando última barra é parcial", () => {
    const q = qualityFlags(makeBars(risingCloses(130), { partialLast: true }), {
      expectedBars: 120,
    });
